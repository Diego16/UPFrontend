...
export const addTask = (title, start, description, user, color) => {
    return (dispatch, getState) => {
        let headers = { "Content-Type": "application/json" };
        let { token } = getState().auth;
        ...
        return fetch(`${base_url}/tasks/`, {
            ...
            .then(res => {
                if (res.status === 201) {
                    return dispatch({ type: 'ADD_TASK', note: res.data });
                } else if (res.status === 401 || res.status === 403) {
                    dispatch({ type: "AUTHENTICATION_ERROR", data: res.data });
                    throw res.data;
                }
...
export const updateSensor = (id, title, start, description, user, color) => {
    return (dispatch, getState) => {
        ...
        return fetch(`${base_url}/tasks/${id}/`, {
            ...
            .then(res => {
                if (res.status === 200) {
                    return dispatch({ type: 'UPDATE_TASK', note: res.data, id });
                } else if (res.status === 401 || res.status === 403) {
                    dispatch({ type: "AUTHENTICATION_ERROR", data: res.data });
                    throw res.data;
                }
...
export const deleteSensor = id => {
    return (dispatch, getState) => {
        ...
        return fetch(`${base_url}/tasks/${id}/`, { headers, method: "DELETE" })
            .then(res => {
                if (res.status === 204) {
                    return { status: res.status, data: {} };
